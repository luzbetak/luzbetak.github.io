...
const CONFIG = {
  ...
  personal: {
    ...
    qualifications: {
      ...
      expertise: 'large-scale systems, cloud architecture, data pipelines, and advanced analytics'
    }
  }
};
...
class MessageBuilder {
  ...
  buildReply(senderName) {
    const greeting = senderName ? `Dear ${senderName},` : 'Dear Hiring Team,';
    
    const message = `${greeting}

Thank you for reaching out regarding this opportunity. I am interested in learning more.

I bring a ${this.config.personal.qualifications.degree}, along with ${this.config.personal.qualifications.experience} specializing in ${this.config.personal.qualifications.expertise}. My experience spans both technical implementation and strategic solution design, enabling me to deliver robust, scalable systems that drive business value.
...
Resume: ${this.config.personal.resume}`;
  ...
  buildShortReply(senderName) {
    const greeting = senderName ? `Hi ${senderName},` : 'Hi,';
    
    const message = `${greeting}

Thank you for your interest. I am open to discussing this opportunity.

I hold a ${this.config.personal.qualifications.degree} and bring ${this.config.personal.qualifications.experience} with expertise in ${this.config.personal.qualifications.expertise}.

I'm based in ${this.config.personal.location} and available for remote work outside my local area. I have a proven track record of success working with distributed teams.

Please share the job details, company name, and work arrangement so we can determine if this is a good mutual fit.
...
function previewReply() {
  ...
  console.log(builder.buildShortReply('Mike'));
}
