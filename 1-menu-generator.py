...
def clean_text(name: str) -> str:
    """Replace - and _ with spaces, preserving original casing."""
    return name.replace("-", " ").replace("_", " ")
    # return " ".join(word.capitalize() for word in name.split()) 

def label_for_index(path: str) -> str:
    """Label for index.html → Parent Folder + 'Index'."""
    parent = os.path.basename(os.path.dirname(path))
    return f"{clean_text(parent)} Index"
...
def generate_submenu(subdir_name: str, subdir_data: dict) -> list:
    ...
    if subdir_data['index']:
        lines.append(f'            <a href="{subdir_data["index"]}">{subdir_name}</a>')
    else:
        # Use the directory path as fallback
        lines.append(f'            <a href="{subdir_data["path"]}">{subdir_name}</a>')
...
def generate_menu_html(directories: dict, output_file: str, backup_dir: str = None):
    ...
    html_lines.extend([
        ...
        '    ',
        '    // Define nav items - order matters! More specific paths first, home last',
        '    const navItems = [',
        '      { path: "/AI_ML",       id: "ai-/-ml" },',
        '      { path: "/Database",    id: "database" },',
        '      { path: "/Snowflake",   id: "snowflake" },',
        '      { path: "/Databricks",  id: "databricks" },',
        '      { path: "/Redshift",    id: "redshift" },',
        '      { path: "/AWS",         id: "aws" },',
        '      { path: "/Programming", id: "programming" },',
        '      { path: "/Info",        id: "info" },',
        ...
        '    }',
        '    ',
        '    // Only highlight Home if no other section matched AND we\'re at root',
        '    if (!matched && currentPath === "/") {',
        '      const homeElement = document.getElementById("home");',
        '      if (homeElement) {',
        '        homeElement.style.backgroundColor = "#004499";',
        '      }',
        '    }',
        '',
