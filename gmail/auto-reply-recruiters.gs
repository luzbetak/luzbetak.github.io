...
const PATTERNS = {
  ...
  // Note: user.dice.com is allowed (recruiter emails), other dice.com blocked
  blockedDomains: [
  ...
  conditionalBlocks: [
    { domain: 'dice.com', allowSubdomain: 'user.dice.com' }
  ]
};
...
I am interested in this opportunity. Please schedule a 20-minute phone call using the link below:
https://calendly.com/kevin-luzbetak/

I am based in Los Angeles County, California. For positions outside my local area, I am available to work remotely.
I hold a Master’s degree in Computer Science and bring over 25 years of experience in software development and data engineering.
...
Phone Call: https://calendly.com/kevin-luzbetak/
...
class EmailProcessor {
  ...
  isBlockedDomain(message) {
    ...
    // Check conditional blocks (e.g., block dice.com but allow user.dice.com)
    for (const rule of this.patterns.conditionalBlocks || []) {
      const domainRegex = new RegExp(`@[^\\s]*${rule.domain.replace(/\./g, '\\.')}`, 'i');
      const allowRegex = new RegExp(`@[^\\s]*${rule.allowSubdomain.replace(/\./g, '\\.')}`, 'i');
      
      if (domainRegex.test(combined) && !allowRegex.test(combined)) {
        return true;
      }
    }
  ...
  isAutomatedMail(message) {
    ...
    // Allow user.dice.com emails (recruiter forwarded emails)
    if (/@user\.dice\.com/i.test(fromStr)) {
      return false;
    }
